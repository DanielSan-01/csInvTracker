...
using (var scope = app.Services.CreateScope())
{
    ...
    try
    {
        ...
        // Workaround: Manually create Goals tables if they don't exist
        Console.WriteLine("Checking for Goals table...");
        await connection.OpenAsync();
        using var checkGoalsCommand = connection.CreateCommand();
        checkGoalsCommand.CommandText = @"
            SELECT EXISTS (
                SELECT FROM information_schema.tables 
                WHERE table_schema = 'public' 
        ...
        ";
        var goalsExists = (bool)(await checkGoalsCommand.ExecuteScalarAsync() ?? false);
        await connection.CloseAsync();
        
        if (!goalsExists)
        {
            Console.WriteLine("Goals tables do not exist. Creating them manually...");
            await dbContext.Database.ExecuteSqlRawAsync(@"
                CREATE TABLE ""Goals"" (
                    ""Id"" uuid NOT NULL,
                    ""CreatedAt"" timestamp with time zone NOT NULL,
                    ...
                    ""SelectedTotal"" decimal(18,2) NOT NULL,
                    ""CoverageTotal"" decimal(18,2) NOT NULL,
                    ""RemainingAmount"" decimal(18,2) NOT NULL,
                    ""SurplusAmount"" decimal(18,2) NOT NULL,
                    ""SkinImageUrl"" text NULL,
                    ""SkinAltImageUrl"" text NULL,
                    ""SkinRarity"" text NULL,
                    ""SkinType"" text NULL,
                    ""SkinWeapon"" text NULL,
                ...
                CREATE TABLE ""GoalSelectedItems"" (
                    ""Id"" integer GENERATED BY DEFAULT AS IDENTITY,
                    ""GoalId"" uuid NOT NULL,
                    ""SourceInventoryItemId"" integer NULL,
                    ""SkinName"" text NOT NULL,
                    ""Price"" decimal(18,2) NOT NULL,
                    ""TradeProtected"" boolean NOT NULL,
                    ""ImageUrl"" text NULL,
                    ""Weapon"" text NULL,
                    ""Type"" text NULL,
                    CONSTRAINT ""PK_GoalSelectedItems"" PRIMARY KEY (""Id""),
                    CONSTRAINT ""FK_GoalSelectedItems_Goals_GoalId"" 
                        FOREIGN KEY (""GoalId"") 
                        REFERENCES ""Goals"" (""Id"") 
                        ON DELETE CASCADE
                );
                CREATE INDEX ""IX_GoalSelectedItems_GoalId"" ON ""GoalSelectedItems"" (""GoalId"");
            ");
            Console.WriteLine("Goals tables created successfully.");
        }
        else
        {
            Console.WriteLine("Goals tables already exist.");
        }
    }
    catch (Exception ex)
    {
